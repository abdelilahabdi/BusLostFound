...
@section('content')
    ...
    <section class="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.06)] sm:p-8">
        <form method="POST" action="{{ route('announcements.update', $announcement) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-wrap items-center gap-3 border-t border-slate-100 pt-6">
                ...
                <a href="{{ route('announcements.my') }}" class="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300">
                    Annuler
                </a>
            </div>
        </form>
