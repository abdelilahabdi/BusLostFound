<x-guest-layout>
    <div class="space-y-7 sm:space-y-8">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-5 sm:space-y-6">
            ...
            <div>
                <x-input-label for="name" class="mb-1.5 text-sm font-semibold text-slate-700" :value="'Nom complet'" />
                <x-text-input id="name" class="block w-full rounded-2xl border-slate-300 px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:border-blue-500 focus:ring-blue-100" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                <x-input-error :messages="$errors->get('name')" class="mt-2 text-xs font-medium text-red-600" />
            </div>

            <div>
                <x-input-label for="email" class="mb-1.5 text-sm font-semibold text-slate-700" :value="'Adresse e-mail'" />
                <x-text-input id="email" class="block w-full rounded-2xl border-slate-300 px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:border-blue-500 focus:ring-blue-100" type="email" name="email" :value="old('email')" required autocomplete="username" />
